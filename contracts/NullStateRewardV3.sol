...
/**
 * @title NullStateRewardV3
 * @notice Reward, burn, and leaderboard management for NullState game — v3
 ...
 * NullStateRewardV2 constrained every season function to seasonId 1..6
 * (`require(_seasonId > 0 && _seasonId <= TOTAL_SEASONS)` in
 * updateLeaderboard / depositSeasonBonus / claimSeasonBonus). But the rest
 * of the system — PassSBTv3 and the whole app — keys seasons by YYYYMM
 * (e.g. 202607 for July 2026). Two consequences made the season-bonus
 * reward IMPOSSIBLE to use on V2:
 *   1. depositSeasonBonus(202607, ...) / updateLeaderboard(202607, ...)
 *      reverted outright (> 6).
 *   2. Even at ids 1..6, claimSeasonBonus fed that id into
 *      passSBT.hasPassForSeason(user, seasonId) — but passes live under
 *      YYYYMM ids on PassSBTv3, so the pass check was ALWAYS false and
 *      every claim reverted "Season Pass required for this season".
 * V2 is a plain (non-upgradeable) contract, so that `<= TOTAL_SEASONS`
 * guard can't be patched in place. V3 is a verbatim copy of V2 with the
 * ONLY change being those three requires relaxed to `require(_seasonId > 0)`
 * so season ids are the same YYYYMM values PassSBTv3 and the app already
 * use. TOTAL_SEASONS is kept as an informational constant but no longer
 * gates anything. Nothing else changed — same tokens, same weekly-burn
 * pool, same rank rewards, same pass gating (which now actually matches).
 *
 * MIGRATION: no user funds live in V2's season pool (it was never funded,
 * precisely because deposits reverted), and passes live on PassSBTv3, not
 * here — so switching is just: deploy V3 with the PassSBTv3 address, point
 * NEXT_PUBLIC_REWARD_CONTRACT_ADDRESS at it, and (per season) publish the
 * top-3 via updateLeaderboard + fund via depositSeasonBonus.
 *
 * WHY V2 EXISTED (inherited context):
...
contract NullStateRewardV3 is Ownable {
    // ============ Constants ============
    ...
    function updateLeaderboard(
        ...
        require(_seasonId > 0, "Invalid season"); // V3: YYYYMM season ids (was capped at TOTAL_SEASONS=6, which broke the whole season-bonus chain -- see header)
        require(
