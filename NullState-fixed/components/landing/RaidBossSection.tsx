...
export default function RaidBossSection() {
    ...
    try {
      ...
      const unsub = onSnapshot(q, snap => {
        ...
        const feed: FeedEntry[] = snap.docs.map(doc => ({
          ...
          time:   'just now',
          type:   doc.data().attackType     ?? 'basic',
        }))
  ...
  // ── Simulate live raid activity (fallback when no real data) ────────────────
  useEffect(() => {
    const interval = setInterval(() => {
      const dmg = Math.floor(Math.random() * 30) + 5
      setBossHp(hp => Math.max(6000, hp - dmg))
      setAttackers(a => a + (Math.random() > 0.7 ? 1 : 0))
      setTweetCount(t => t + (Math.random() > 0.6 ? 1 : 0))

      const players = ['0xe2F0..', '0xf3G1..', '0xa7H2..', '0xb8I3..', '0xc9J4..']
      const types   = ['tweet', 'artifact', 'basic', 'tweet']
      const newAtk: FeedEntry = {
        player: players[Math.floor(Math.random() * players.length)],
        damage: Math.floor(Math.random() * 50) + 10,
        time:   'just now',
        type:   types[Math.floor(Math.random() * types.length)],
      }
      setRecentAttacks(prev => [newAtk, ...prev.slice(0, 3)])
    }, 2500)
  ...
  const handleTweetAttack = async () => {
    ...
    try {
      const res = await fetch('/api/tweet', {
        ...
        body: JSON.stringify({
          action:    'get-tweet-intent',
          bossName:  'THE 51%',
          bossHp,
          damage:    Math.floor(Math.random() * 40) + 25,
          playerName: 'BRAVE_WARRIOR',
        }),
      })
      const data = await res.json()
      if (data.intentUrl) window.open(data.intentUrl, '_blank', 'width=600,height=400')

      // Log to Firebase feed
      try {
      ...
      const fallbackText = `⚔️ I'm attacking THE 51% in @NullStateRPG!\n\n🔴 Boss HP: ${bossHp.toLocaleString()}/10,000\n\nJoin the raid — tweet = attack!\n#NullState #CeloChronicles #Web3RPG`
      window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(fallbackText)}`, '_blank', 'width=600,height=400')
    } finally {
      setTimeout(() => setTweeting(false), 1500)
    }
  }
  ...
  return (
    <section id="raid" className="relative py-24 z-[2] overflow-hidden">
      <style>{`
        ...
        @keyframes bossGlow {
          0%,100% { filter: drop-shadow(0 0 16px rgba(153,0,204,0.6)); }
          50%      { filter: drop-shadow(0 0 32px rgba(255,0,255,0.9)); }
        }
        .boss-idle { animation: bossFloat 2.5s ease-in-out infinite, bossGlow 2s ease-in-out infinite; }
      `}</style>
          ...
          <div className="reveal" style={{ transitionDelay: '0.1s' }}>
            <div className="border border-[rgba(255,34,68,0.3)] bg-[rgba(255,34,68,0.03)] p-6 relative overflow-hidden">
              ...
              <div className="text-center mb-6">
                <div className="inline-block boss-idle">
                  <img
