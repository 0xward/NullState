  ...
  const updatePlayerProgressFn = useCallback(
    async (xp: number, level: number, kills: number) => {
      ...
      try {
        ...
        if (receipt.status === 'success') {
          // The on-chain tx only ever bumps the contract's kill counter by
          // 0/1 (enemyKilled is a bool) — fold the REAL number of kills this
          // life into the off-chain leaderboard total so "Souls Purged"
          // this run isn't lost. See recordRunKills() for how repeated
          // deaths within one continuous (Revive-chained) life are
          // deduplicated so kills aren't counted twice.
          if (walletAddress) {
            await recordRunKills(walletAddress, kills)
          }
          await fetchPlayerProfile()
        } else {
