...
function WalletExtrasProvider({ children }: { children: ReactNode }) {
  ...
  const executeAction = useCallback(
    async (params: ExecuteActionParams): Promise<string> => {
      ...
      // Clamp to the contract's hard require() ranges (damageDealt <= 150,
      // damageReceived <= 100, xpGained <= 200) so the tx can never revert
      // with "Damage out of range" / "XP out of range" on big boss strikes.
      const dmg  = Math.min(150, Math.max(0, Math.round(damageDealt)))
      const dmgR = Math.min(100, Math.max(0, Math.round(damageReceived)))
      const xp   = Math.min(200, Math.max(0, Math.round(xpGained)))

      const data = encodeFunctionData({
        abi:          NULLSTATE_ABI,
        functionName: 'executeAction',
        args: [actionType, dmg, dmgR, BigInt(xp), enemyKilled],
      })
  ...
  const attackRaid = useCallback(async (damage: number): Promise<string> => {
    ...
    const data = encodeFunctionData({
      ...
      // Contract enforces damage in [1, 200]
      args:         [Math.min(200, Math.max(1, Math.round(damage)))],
    })
    return sendTx(data, ACTION_COST_WEI)
  }, [sendTx])
